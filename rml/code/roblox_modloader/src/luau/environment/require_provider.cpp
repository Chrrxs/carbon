...
namespace rml::luau::environment
{
	...
	bool RequireProvider::register_globals(lua_State* L) noexcept
	{
		try
		{
			...
			if (lua_isfunction(L, -1))
			{
				...
				{
					LOG_WARN("Failed to store original require function reference for lua_State: {}", static_cast<void*>(L));
				}
			...
			{
				lua_pop(L, 1);
				LOG_WARN("Original require function not found or not a function for lua_State: {}", static_cast<void*>(L));
			}
	...
	int RequireProvider::custom_require(lua_State* L)
	{
		try
		{
			...
			if (const auto cache_it = module_cache.find(module_name); cache_it != module_cache.end())
			{
			...
			if (resolved_path.empty() || !file_exists(resolved_path))
			{
				if (module_name.starts_with("@self/"))
				{
					...
					const std::string expected_path = std::format("{}/scripts/{}.[lua,luau]", mod_path, module_name.substr(6));
			...
			ScriptContext::set_thread_identity(L, RBX::Security::Permissions::RobloxEngine, RBX::Security::FULL_CAPABILITIES);
			...
			if (g_pointers->m_roblox_pointers.luau_load(L, chunk_name.c_str(), bytecode.data(), bytecode.size(), 0) != 0)
			{
				lua_error(L);
				return 0;
			}

			ScriptContext::elevate_closure(static_cast<const Closure*>(lua_topointer(L, -1)), RBX::Security::FULL_CAPABILITIES);
	...
	{
		if (auto self_relative = resolve_self_path(module_name, L); !self_relative.empty() && file_exists(self_relative))
		{
	...
	std::string RequireProvider::resolve_rml_path(const std::string& module_name)
	{
		try
		{
			const auto rml_dir = utils::directory::get_mod_loader_directory() / "libraries";

			for (const std::vector<std::string> extensions = {".lua", ".luau"}; const auto& ext : extensions)
			{
				if (std::filesystem::path full_path = rml_dir / (module_name + ext); std::filesystem::exists(full_path))
				{
