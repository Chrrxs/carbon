...
if ($NativeOnly -and -not (Test-Path -LiteralPath $BuildDir -PathType Container)) {
...
Write-Host "[fresh-rml] Checking isolated .NET SDK"
...
if ($NativeOnly) {
    Write-Host "[fresh-rml] Validating cached .NET SDK in native-only mode"
    if (-not (Test-Path -LiteralPath $dotnet -PathType Leaf) -or -not (Test-Path -LiteralPath $installedDotnetSdk -PathType Container)) {
        throw "isolated .NET SDK was not found in $dotnetRoot for native-only mode; run full mode first"
    }
} else {
...
if ($NativeOnly) {
    Write-Host "[fresh-rml] Validating cached helper and managed artifacts for native-only mode"
    $stageHelper = Join-Path $stage "carbon-studio-helper.exe"
    if (-not (Test-Path -LiteralPath $helperOut -PathType Leaf) -and -not (Test-Path -LiteralPath $stageHelper -PathType Leaf)) {
        throw "cached helper executable carbon-studio-helper.exe was not found in $BuildDir; run full mode first"
    }

    $requiredManagedInStage = @(
    ...
    $missingCache = $requiredManagedInStage | Where-Object { -not (Test-Path -LiteralPath $_ -PathType Leaf) }
    if ($missingCache) {
        throw "cached managed artifact is missing in $BuildDir for native-only mode:`n$($missingCache -join "`n"); run full mode first"
    }
}
...
if (-not $NativeOnly) {
    ...
    Invoke-Checked -Program $clCompiler -Arguments @(
        ...
        "/link", "/Brepro", "/INCREMENTAL:NO", "kernel32.lib", "user32.lib", "advapi32.lib", "winhttp.lib"
    )
...
if (Test-Path -LiteralPath $helperOut -PathType Leaf) {
