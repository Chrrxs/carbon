...
use rbx_dom_weak::{
	types::{
		...
		Content, ContentId, ContentType, Enum, EnumItem, Faces, Font, MaterialColors, Matrix3, NetAssetRef,
		NumberRange, NumberSequence, NumberSequenceKeypoint, PhysicalProperties, Ray, Rect, Ref, SecurityCapabilities,
		SharedString, Tags, UDim, UDim2, UniqueId, Variant, VariantType, Vector2, Vector3, Vector3int16,
	},
	Ustr, UstrMap,
};

use rbx_reflection::{
	ClassDescriptor, ClassTag, PropertyKind, PropertyMigration, PropertySerialization, ReflectionDatabase,
};

use crate::{
	chunk::ChunkBuilder,
	core::{
		find_property_descriptors, PropertyDescriptors, RbxWriteExt, FILE_MAGIC_HEADER, FILE_SIGNATURE, FILE_VERSION,
	},
	types::Type,
	Serializer,
};
...
use super::{CompressionType, InstanceSource, InstanceView};

static FILE_FOOTER: &[u8] = b"</roblox>";

pub(super) fn write_stream_header<W: Write>(
	output: &mut W,
	type_count: u32,
	instance_count: u32,
) -> Result<(), InnerError> {
	...
	output.write_le_u16(FILE_VERSION)?;
	output.write_le_u32(type_count)?;
	output.write_le_u32(instance_count)?;
	output.write_all(&[0; 8])?;
	Ok(())
}

pub(super) fn write_stream_shared_strings<W: Write>(
	output: &mut W,
	...
	let mut chunk = ChunkBuilder::new(b"SSTR", compression);
	chunk.write_le_u32(0)?;
	chunk.write_le_u32(values.len() as u32)?;
	for value in values {
		chunk.write_all(&[0; 16])?;
		chunk.write_binary_string(value.data())?;
	}
	chunk.dump(output)?;
	Ok(())
}

pub(super) fn write_stream_metadata<W: Write>(
	output: &mut W,
	compression: CompressionType,
	...
	let mut chunk = ChunkBuilder::new(b"META", compression);
...
pub(super) fn write_stream_parents<W: Write>(
	output: &mut W,
	compression: CompressionType,
	relations: &[(i32, i32)],
) -> Result<(), InnerError> {
	let mut chunk = ChunkBuilder::new(b"PRNT", compression);
	chunk.write_u8(0)?;
	chunk.write_le_u32(relations.len() as u32)?;
	chunk.write_referent_array(relations.iter().map(|(subject, _)| *subject))?;
	chunk.write_referent_array(relations.iter().map(|(_, parent)| *parent))?;
	chunk.dump(output)?;
	Ok(())
}

pub(super) fn write_stream_end<W: Write>(output: &mut W) -> Result<(), InnerError> {
...
pub(super) struct SerializerState<'dom, 'db, W> {
	...
	/// The source containing all instances being serialized.
	source: &'dom dyn InstanceSource,
...
struct TypeInfo<'dom, 'db> {
	...
	instances: Vec<InstanceView<'dom>>,
...
impl<'dom> PropInfo<'dom> {
	...
	fn extend_with_default(&mut self, desired_len: usize) {
		...
		self.values.extend(core::iter::repeat_n(self.default_value, additional));
	}
	/// Set the migration
	fn set_migration(&mut self, m_new: &'dom PropertyMigration<'dom>) {
...
fn studio_compat_service_format(class: &str) -> Option<bool> {
	match class {
		// These Studio 0.729 classes are not present in the published 0.728
		// reflection database. Their INST object format still has to match their
		// 0.729 Service tags so Roblox consumes the following service marker byte.
		"DeviceDisplayService" | "PopLatencyService" => Some(true),
		"DisplayWakeLock" => Some(false),
		_ => None,
	}
}
...
impl<'dom, 'db> TypeInfos<'dom, 'db> {
	...
	fn get_or_create(&mut self, class: Ustr) -> &mut TypeInfo<'dom, 'db> {
		if let btree_map::Entry::Vacant(entry) = self.values.entry(class) {
			...
			} else if let Some(is_service) = studio_compat_service_format(class.as_str()) {
				is_service
			} else {
	...
	fn new(
		...
		match find_property_descriptors(database, class_descriptor, &prop_name) {
			Some((superclass_descriptor, descriptors)) => {
				...
				let descriptor = descriptors.serialized?;
				let serialized = if let PropertyKind::Canonical {
				...
				} = &descriptor.kind
				{
					...
					let [new_property_name] = prop_migration.new_property_names() else {
						// Canonical source never contains multi-target migration sources.
						return None;
					};
...
impl<'dom, 'db: 'dom> TypeInfo<'dom, 'db> {
	...
	fn push_prop_info_with_names(
		...
		for prop_name in prop_names {
			self.properties_visited.insert(prop_name, Some(logical_index));
		}
		prop_info
	}
	...
	fn get_or_create_logical_property<'a>(
		...
		let Some(ser_info) = SerializationInfo::new(class, database, prop_name, sample_value) else {
...
impl<'dom, 'db: 'dom, W: Write> SerializerState<'dom, 'db, W> {
	pub fn new(serializer: &'db Serializer<'db>, source: &'dom dyn InstanceSource, output: W) -> Self {
		SerializerState {
			serializer,
			source,
			output,
	...
	pub(super) fn seed_referents(&mut self, referents: &[(Ref, i32)]) {
		self.id_to_referent.extend(referents.iter().copied());
	}

	pub(super) fn seed_shared_strings(&mut self, shared_strings: &[(SharedString, u32)]) {
		self.shared_string_ids.extend(shared_strings.iter().cloned());
	}

	pub(super) fn set_single_type_id(&mut self, type_id: u32) {
		assert_eq!(self.type_infos.values.len(), 1);
		self.type_infos.values.values_mut().next().unwrap().type_id = type_id;
	}

	pub(super) fn instance_positions(
		&self,
		indexed_refs: &std::collections::HashSet<Ref>,
	) -> std::collections::HashMap<Ref, super::SerializedInstancePosition> {
		let mut positions = std::collections::HashMap::with_capacity(indexed_refs.len());
		for (class_name, type_info) in &self.type_infos.values {
			for (index, instance) in type_info.instances.iter().enumerate() {
				if indexed_refs.contains(&instance.referent) {
					positions.insert(
						instance.referent,
						super::SerializedInstancePosition {
	...
	pub fn add_instances(&mut self, referents: &[Ref]) -> Result<(), InnerError> {
		...
		while let Some(referent) = to_visit.last() {
			let instance = self
				.source
				.get_by_ref(*referent)
				.ok_or(InnerError::InvalidInstanceId { referent: *referent })?;
			...
			while let Some(referent) = to_visit.last() {
				...
				if !instance.children.is_empty() && instance.children.last() != last_visited_child.as_ref() {
					break;
				}
		...
		log::debug!("Discovered {} unique TypeInfos", self.type_infos.values.len());
	...
	#[profiling::function]
	pub fn collect_type_info(&mut self, instance: InstanceView<'dom>) -> Result<(), InnerError> {
		let SerializerState {
	...
	pub fn write_header(&mut self) -> Result<(), InnerError> {
		...
		self.output.write_le_u32(self.type_infos.values.len() as u32)?;
		self.output.write_le_u32(self.relevant_instances.len() as u32)?;
	...
	pub fn serialize_metadata(&mut self) -> Result<(), InnerError> {
		log::trace!("Writing metadata");
		write_stream_metadata(&mut self.output, self.serializer.compression, &self.serializer.metadata)
	}
	...
	/// chunks named PROP.
	#[profiling::function]
	pub fn serialize_properties(&mut self) -> Result<super::SerializationReport, InnerError> {
		log::trace!("Writing properties");

		let mut report = super::SerializationReport::default();
		...
		for (type_name, type_info) in &mut self.type_infos.values {
			...
			type_info.properties.sort_by_key(|prop_info| prop_info.canonical_name);
			...
			for prop_info in &mut type_info.properties {
				prop_info.extend_with_default(type_info.instances.len());
			}

			let instances = &type_info.instances;
			let properties = &type_info.properties;
			let type_id = type_info.type_id;
			let compression = self.serializer.compression;
			let source = self.source;
			let id_to_referent = &self.id_to_referent;
			let shared_string_ids = &self.shared_string_ids;
			let jobs = properties.len() + 1;
			let workers = rayon::current_num_threads()
				.min(self.serializer.property_workers)
				.max(1);
			let parallel = workers > 1 && instances.len() >= 16 * 1024 && jobs > 1;
			let window = if parallel { workers } else { 1 };
			let mut first = 0;
			while first < jobs {
				let end = (first + window).min(jobs);
				let encode = |job: usize| -> Result<(Vec<u8>, u64, Option<Ustr>), InnerError> {
					...
					if job == name_index {
						...
						for &instance in instances {
							if let Some(raw_name) = instance.raw_name {
								properties += u64::from(std::str::from_utf8(raw_name).is_err());
								chunk.write_binary_string(raw_name)?;
							} else {
								chunk.write_string(instance.name)?;
							}
						}
						return Ok((chunk.encode()?, properties, None));
					}

					let property_index = if job < name_index { job } else { job - 1 };
					let prop_info = &properties[property_index];
					...
					Ok((chunk.encode()?, instances.len() as u64, Some(prop_info.canonical_name)))
				};
				let encoded = if parallel {
					(first..end).into_par_iter().map(encode).collect::<Vec<_>>()
				} else {
					(first..end).map(encode).collect::<Vec<_>>()
				};
				for encoded in encoded {
					let (bytes, properties, column) = encoded?;
					report.properties += properties;
					if let Some(column) = column {
						report.property_columns.push((*type_name, column));
					}
			...
			fn write_prop_info<'dom>(
				...
				// this chunk.
				let type_mismatch = |i: usize, bad_value: &Variant, valid_type_names: &'static str| {
					Err(InnerError::PropTypeMismatch {
						type_name: type_name.to_string(),
						prop_name: prop_info.canonical_name.to_string(),
						valid_type_names,
						actual_type_name: format!("{:?}", bad_value.ty()),
						instance_full_name: full_name_for(source, instances[i].referent),
					})
				};
				...
				match prop_type {
						...
						for (i, rbx_value) in values {
							match rbx_value {
								...
								Variant::Attributes(value) => {
									...
									value.to_writer(&mut buf).map_err(|_| invalid_value(i, rbx_value))?;
					...
					Type::CFrame => {
						...
						for matrix in rotations {
							if let Some(id) = exact_basic_rotation_id(&matrix) {
								chunk.write_u8(id)?;
							} else {
								chunk.write_u8(0x00)?;
						...
						for (i, rbx_value) in values {
							if let Variant::SharedString(value) = rbx_value {
								if let Some(id) = shared_string_ids.get(value) {
									...
									panic!("SharedString {} was not found during type collection", value.hash())
								}
								...
								if let Some(id) = shared_string_ids.get(sstr_ref) {
									...
									panic!("NetAssetRef {} was not found during type collection", value.hash());
								}
							} else {
	...
	pub fn serialize_parents(&mut self) -> Result<(), InnerError> {
		...
		let object_referents = self.relevant_instances.iter().map(|id| self.id_to_referent[id]);

		let parent_referents = self.relevant_instances.iter().map(|id| {
			let instance = self.source.get_by_ref(*id).unwrap();
			...
			if instance.parent.is_some() {
				self.id_to_referent.get(&instance.parent).cloned().unwrap_or(-1)
			} else {
				-1
			}
		});
...
fn exact_basic_rotation_id(matrix: &Matrix3) -> Option<u8> {
	let id = matrix.to_basic_rotation_id()?;
	(Matrix3::from_basic_rotation_id(id).ok()? == *matrix).then_some(id)
}

/// Equivalent to Instance:GetFullName() from Roblox.
fn full_name_for(source: &dyn InstanceSource, subject_ref: Ref) -> String {
	let mut components = Vec::new();
	let mut current_id = subject_ref;

	while current_id.is_some() {
		let instance = source.get_by_ref(current_id).unwrap();
		components.push(instance.name);
...
fn fallback_default_value(rbx_type: VariantType) -> Option<&'static Variant> {
	...
	static DEFAULT_UDIM2: Variant = Variant::UDim2(UDim2::new(UDim::new(0.0, 0), UDim::new(0.0, 0)));
	static DEFAULT_RAY: Variant = Variant::Ray(Ray::new(Vector3::new(0.0, 0.0, 0.0), Vector3::new(0.0, 0.0, 0.0)));
	...
	static DEFAULT_RECT: Variant = Variant::Rect(Rect::new(Vector2::new(0.0, 0.0), Vector2::new(0.0, 0.0)));
	static DEFAULT_PHYSICALPROPERTIES: Variant = Variant::PhysicalProperties(PhysicalProperties::Default);
	...
	static DEFAULT_SECURITYCAPABILITIES: Variant = Variant::SecurityCapabilities(SecurityCapabilities::from_bits(0));
