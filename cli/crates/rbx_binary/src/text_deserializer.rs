...
use rbx_dom_weak::types::{
	Axes, BrickColor, CFrame, Color3, Color3uint8, ColorSequence, ColorSequenceKeypoint, CustomPhysicalProperties,
	Enum, Faces, Font, FontStyle, FontWeight, Matrix3, NumberRange, NumberSequence, NumberSequenceKeypoint,
	PhysicalProperties, Ray, Rect, SecurityCapabilities, SharedString, UDim, UDim2, UniqueId, Vector2, Vector3,
	Vector3int16,
};
	...
	pub fn from_reader<R: Read>(mut reader: R) -> Self {
		...
		loop {
			...
			match &chunk.name {
				...
				b"INST" => chunks.push(decode_inst_chunk(chunk.data.as_slice(), &mut count_by_type_id)),
				b"PROP" => chunks.push(decode_prop_chunk(chunk.data.as_slice(), &mut count_by_type_id)),
...
fn decode_inst_chunk<R: Read>(mut reader: R, count_by_type_id: &mut HashMap<u32, usize>) -> DecodedChunk {
	...
	let referents = reader.read_referent_array(num_instances as usize).unwrap().collect();
...
fn decode_prop_chunk<R: Read>(mut reader: R, count_by_type_id: &mut HashMap<u32, usize>) -> DecodedChunk {
	let type_id = reader.read_le_u32().unwrap();
...
impl DecodedValues {
	fn decode<R: Read>(mut reader: R, prop_count: usize, prop_type: Type) -> Option<Self> {
		match prop_type {
			...
			Type::Int32 => {
				let values = reader.read_interleaved_i32_array(prop_count).unwrap().collect();

				Some(DecodedValues::Int32(values))
			}
			Type::Float32 => {
				let values = reader.read_interleaved_f32_array(prop_count).unwrap().collect();
			...
			Type::UDim2 => {
				...
				let x_values = scale_x.zip(offset_x).map(|(scale, offset)| UDim::new(scale, offset));
				let y_values = scale_y.zip(offset_y).map(|(scale, offset)| UDim::new(scale, offset));

				let values = x_values.zip(y_values).map(|(x, y)| UDim2::new(x, y)).collect();
			...
			Type::Font => {
				...
				for _ in 0..prop_count {
					...
					let weight = FontWeight::from_u16(reader.read_le_u16().unwrap()).unwrap_or_default();
			...
			Type::Color3 => {
				...
				let values = r.zip(g).zip(b).map(|((r, g), b)| Color3::new(r, g, b)).collect();
			...
			Type::Vector3 => {
				...
				let values = x.zip(y).zip(z).map(|((x, y), z)| Vector3::new(x, y, z)).collect();
			...
			Type::Int64 => {
				let values = reader.read_interleaved_i64_array(prop_count).unwrap().collect();

				Some(DecodedValues::Int64(values))
			}
			Type::SharedString => {
				let values = reader.read_interleaved_u32_array(prop_count).unwrap().collect();
			...
			Type::Content => {
				...
				let mut objects: VecDeque<i32> = reader.read_referent_array(object_count).unwrap().collect();
				...
				for (v, ty) in values.iter_mut().zip(source_types) {
					*v = match ty {
						...
						2 => SerializedContentType::Object(objects.pop_front().unwrap()),
