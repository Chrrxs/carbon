...
use rbx_reflection::{ClassDescriptor, PropertyDescriptor, PropertyKind, PropertySerialization, ReflectionDatabase};
...
pub trait RbxReadExt: Read {
	...
	fn read_interleaved_bytes<const N: usize>(&mut self, len: usize) -> io::Result<ReadInterleavedBufferIter<N>> {
	...
	fn read_interleaved_i32_array(&mut self, len: usize) -> io::Result<impl Iterator<Item = i32> + use<Self>> {
		Ok(self
	...
	/// The bytes are read into a buffer immediately,
	/// and the values are transformed during iteration.
	fn read_interleaved_u32_array(&mut self, len: usize) -> io::Result<impl Iterator<Item = u32> + use<Self>> {
	...
	fn read_interleaved_f32_array(&mut self, len: usize) -> io::Result<impl Iterator<Item = f32> + use<Self>> {
		Ok(self
			.read_interleaved_bytes(len)?
	...
	fn read_referent_array(&mut self, len: usize) -> io::Result<impl Iterator<Item = i32> + use<Self>> {
		let mut last = 0;
		Ok(self.read_interleaved_i32_array(len)?.map(move |mut referent| {
			referent += last;
			last = referent;
			referent
		}))
	...
	/// The bytes are read into a buffer immediately,
	/// and the values are transformed during iteration.
	fn read_interleaved_i64_array(&mut self, len: usize) -> io::Result<impl Iterator<Item = i64> + use<Self>> {
		Ok(self
			.read_interleaved_bytes(len)?
...
impl ChunkBuilder {
	...
	where
		...
		<I as IntoIterator>::IntoIter: ExactSizeIterator,
	{
		self.write_interleaved_bytes(values.into_iter().map(|v| v.to_bits().rotate_left(1).to_be_bytes()))
	}
	...
	pub fn new(
		...
		match &property_descriptor.kind {
			...
			PropertyKind::Canonical { serialization } => {
				let serialized = find_serialized_from_canonical(class_descriptor, property_descriptor, serialization);
			...
			PropertyKind::Alias { alias_for } => {
				...
				if let PropertyKind::Canonical { serialization } = &canonical.kind {
					let serialized = find_serialized_from_canonical(class_descriptor, canonical, serialization);

					Some(PropertyDescriptors { canonical, serialized })
				} else {
...
pub fn find_property_descriptors<'db>(
	...
	let (class, prop) = database.superclasses_iter(class_descriptor).find_map(|class| {
...
fn find_serialized_from_canonical<'db>(
	...
	match serialization {
		...
		PropertySerialization::Serializes | PropertySerialization::Migrate { .. } => Some(canonical),
