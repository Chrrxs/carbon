...
use crate::{carbon_error, logger, util};
...
pub fn hook() {
	panic::set_hook(Box::new(|panic_info| {
		...
		let mut report = String::from("> This crash report was automatically generated by Carbon");
		report.push_str("\n\nDetails:\n--------\n");
		report.push_str("*Provide extra details about your problem or what happened here*");

		carbon_error!("{}", "Carbon has crashed!".bold());

		report.push_str("\n\nMessage:\n----------\n");
		report.push_str(message);

		carbon_error!("{}: {}", "Message".bold(), message);
		...
		if let Some(location) = panic_info.location() {
			...
			carbon_error!("{}: {}: {}", "Location".bold(), location.file(), location.line());
		} else {
		...
		if verbosity != LevelFilter::Trace {
			report.push_str("Verbose output disabled, run Carbon with `-vvvv` flag to enable");

			carbon_error!(
				"{}: Run Carbon with {} flag to show detailed tracing",
				"Verbosity".bold(),
				"-vvvv".bold()
			);
		} else {
		...
		if backtrace_enabled {
			...
			report.push_str("Backtrace disabled, run Carbon with `--backtrace` flag to enable");

			carbon_error!(
				"{}: Run Carbon with {} flag to show full backtrace\n",
				"Backtrace".bold(),
				"--backtrace".bold()
			);
		}
		...
		if report_issue {
			...
			url.push_str(&format!("/issues/new?title=Carbon crashed: {message}&body="));
