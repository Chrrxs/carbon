...
use crate::{
	carbon_info,
	config::{Config as CarbonConfig, ConfigKind},
	ext::PathExt,
	logger, util,
};
...
impl Config {
	pub fn main(self) -> Result<()> {
		let config = CarbonConfig::new();

		let config_kind = match self.config.unwrap_or_default() {
			ConfigType::Default => ConfigKind::Default,
			ConfigType::Global => ConfigKind::Global(util::get_carbon_dir()?.join("config.toml")),
			ConfigType::Workspace => ConfigKind::Workspace(env::current_dir()?.join("carbon.toml")),
		};
		...
		{
			drop(config);
			CarbonConfig::load_virtual(config_kind)?;
		} else {
			drop(config);
		};
		...
		if self.list {
			carbon_info!("List of all available config options:\n\n{}", config.list());
		...
		match (self.setting, self.value) {
			(Some(setting), Some(value)) => {
				drop(config);
				let mut config = CarbonConfig::new_mut();
			...
			(Some(setting), None) => {
				let default = CarbonConfig::default();
			...
			_ => {
				...
				carbon_info!("Opened config file. Manually go to: {}", config_path.to_string().bold());
