...
// These Carbon logs ignore verbosity level, aside of `Off`
#[macro_export]
macro_rules! carbon_error {
    ($($arg:tt)+) => (log::log!(target: "carbon_log", log::Level::Error, $($arg)+))
}

#[macro_export]
macro_rules! carbon_warn {
    ($($arg:tt)+) => (log::log!(target: "carbon_log", log::Level::Warn, $($arg)+))
}

#[macro_export]
macro_rules! carbon_info {
    ($($arg:tt)+) => (log::log!(target: "carbon_log", log::Level::Info, $($arg)+))
}
...
pub fn init(verbosity: LevelFilter, log_style: WriteStyle) {
	...
	builder.format(move |buffer, record| {
		if record.level() > verbosity && record.target() != "carbon_log" {
			return Ok(());
		}
...
mod tests {
	...
	#[test]
	fn managed_launch_failure_renders_the_complete_cause_chain() {
		let error = Err::<(), _>(anyhow::anyhow!("connection closed before message completed"))
			.context("robloxstudio-mcp launch request may have been dispatched")
			.context("failed to launch the managed Roblox Studio place")
			.unwrap_err();

		let rendered = render_error(&error);
		assert!(rendered.contains("failed to launch the managed Roblox Studio place"));
		assert!(rendered.contains("robloxstudio-mcp launch request may have been dispatched"));
